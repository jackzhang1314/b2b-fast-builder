...
获客参考与视觉参考分开 → 记录设计方向 → 用正式静态生成器做三个样板 → 桌面/手机截图自查 → 用户反馈与定点修改 → 确认后扩展全站。

- `DESIGN.md` 记录方向、批准范围和规则，具体视觉值由代码中的语义 Token 维护。
- 产品图片保留真实身份与素材来源；产品完整展示、场景焦点裁切，支持 local/R2 统一素材 ID。
...
页面建好后，Agent 主动接着带客户完成：Cloudflare 注册授权 → 域名配置 → Resend 发信验证 → 询盘邮件测试 → 按需接入 Tidio 聊天。

- 必要工具先检查，缺失时从官方来源安装；用户不用自己知道 CLI/Skill 名称。
